...
# 关节角度
dof_pos[:] = [-0.1, 2.3, -2.4, 0.1, 2.3, -2.4, -0.1, 2.3, -2.4, 0.1, 2.3, -2.4]
# 足端位置
# 计算足端位置在质心坐标系的坐标
for i in range(toe_pos.shape[0]):
    ...
    toe_pos[i, 6:9] = np.transpose(casadi.DM(panda7.transrpy(dof_pos[i, :3], 2, [0, 0, 0], [0, 0, 0]) @ panda7.toe).full()[:3])
    toe_pos[i, 9:12] = np.transpose(casadi.DM(panda7.transrpy(dof_pos[i, :3], 3, [0, 0, 0], [0, 0, 0]) @ panda7.toe).full()[:3])
...
# # 导出完整轨迹
outfile = 'output_panda/panda_down.txt'
np.savetxt(outfile, ref, delimiter=',')

# # 导出fixed arm轨迹
outfile = 'output_panda_fixed_arm/panda_down.txt'
np.savetxt(outfile, ref[:, :49], delimiter=',')

# 导出 fixed gripper轨迹
outfile = 'output_panda_fixed_gripper/panda_down.txt'
