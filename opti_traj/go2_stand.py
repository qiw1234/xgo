...
# 关节角度
dof_pos[:] = [-0.1, 0.8, -1.5, 0.1, 0.8, -1.5, -0.1, 0.8, -1.5, 0.1, 0.8, -1.5]
# 足端位置
for i in range(toe_pos.shape[0]):
    ...
    toe_pos[i, 6:9] = np.transpose(casadi.DM(go2.transrpy(dof_pos[i, 6:9], 2, [0, 0, 0], [0, 0, 0]) @ go2.toe).full()[:3])
    toe_pos[i, 9:12] = np.transpose(casadi.DM(go2.transrpy(dof_pos[i, 9:12], 3, [0, 0, 0], [0, 0, 0]) @ go2.toe).full()[:3])
...
with open('go2ST/stand.json', 'w') as f:
